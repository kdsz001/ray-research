...
# Helper：运行一段 python 代码并捕获输出 + 返回值
run_check() {
  local name="$1"
  local code="$2"
  local output
  ...
  local rc=$?
  if [ $rc -ne 0 ]; then
    echo "❌ 检查 [$name] 脚本本身出错（rc=$rc）：$output" >&2
    SCRIPT_FAIL=1
    return 1
  fi
  printf '%s' "$output"
...
MISSING_FILES=$(run_check "missing-files" '
import json, os
...
ORPHAN_FILES=$(run_check "orphan-files" '
import json, os
m = json.load(open("manifest.json"))
in_manifest = set()
for t in m.get("topics", []):
...
MISSING_INDEX=$(run_check "missing-index" '
import json, os
m = json.load(open("manifest.json"))
missing = []
for t in m.get("topics", []):
...
SIDEBAR_MISSING=$(run_check "sidebar-injection" '
import json, os
m = json.load(open("manifest.json"))
missing = []
for t in m.get("topics", []):
...
# 总结：脚本错误优先于内容错误，避免假 PASS
if [ $SCRIPT_FAIL -ne 0 ]; then
  echo "❌ 检查脚本本身出错，结果不可信。请检查 python3 是否可用、临时目录是否可写。"
  exit 2
fi
